...
export function ManualSeedInstructions() {
  const [isOpen, setIsOpen] = useState(false)
  const [copied, setCopied] = useState(false)

  const sqlScript = `-- This SQL script can be run directly in the Supabase SQL Editor
      ...
      <div className="flex items-center justify-between">
        <CollapsibleTrigger asChild>
          <Button variant="ghost" className="flex items-center p-0 hover:bg-transparent">
            ...
            <span className="text-lg font-medium">Manual Seeding Instructions</span>
          </Button>
        </CollapsibleTrigger>
      </div>
      <CollapsibleContent className="mt-4">
        <div className="space-y-4">
          <p className="text-gray-400">
            If the automatic seeding process is not working, you can manually seed the database by running the following
            SQL script in the Supabase SQL Editor:
          </p>

          <div className="relative">
            <pre className="p-4 bg-[#0D0D0D] border border-[#2E2E2E] rounded-md overflow-x-auto text-sm text-gray-300 max-h-96">
              {sqlScript}
            </pre>
            <Button size="sm" variant="ghost" className="absolute top-2 right-2 h-8 w-8 p-0" onClick={handleCopy}>
          ...
          <div className="space-y-2">
            <h3 className="text-md font-medium">Steps to manually seed the database:</h3>
            <ol className="list-decimal list-inside space-y-2 text-gray-400">
              ...
              <li>Verify that the tables have been created and populated</li>
            </ol>
          </div>
        </div>
      </CollapsibleContent>
